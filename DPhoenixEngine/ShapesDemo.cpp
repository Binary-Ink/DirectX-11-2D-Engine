...
public:
	...
	//new method / specifically handels individual sprite rendering
	void RenderColorPrimitives(DPhoenix::PrimitiveInstance* model,
...
bool ShapesDemo::Init(bool fullScreen)
{
	...
	mColorBox->LoadColorInstance(md3dDevice, XMFLOAT4(1.0f, 1.0f, 1.0f, 1.0f), mBox);

	mColorBox->mPosition.x = 10.0f;
...
void ShapesDemo::OnResize()
{
	...
	//window can be resized so aspect ratio needs recalculating and project matrix
...
#pragma region RenderColorPrimitives
	...
	//set up vertex input
	//(would need to change for different kinds of rendering) 
	md3dImmediateContext->IASetInputLayout(DPhoenix::InputLayouts::PosColor);

	//here we will render the box with our new method
	//we send pointer to the instances and also use  color rendering techniques (color.fx)
	RenderColorPrimitives(mColorBox, activeColorTech);
...
//to render sprite need to disable depth testing (so it layers)
...
#pragma region RenderFonts
	...
	debugText << "Scale x: " << mColorBox->mScale.x << 
